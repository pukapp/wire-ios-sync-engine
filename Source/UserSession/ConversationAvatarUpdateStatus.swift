//
//  ConversationAvatarUpdateStatus.swift
//  WireSyncEngine-ios
//
//  Created by 老西瓜 on 2019/12/11.
...
/**
 新增群头像的上传状态跟踪 - 仿照UserProfileImageUpdateStatus
 1. 根据选择的图片data进行处理(preprocess)，获取preview，complete两种尺寸的data
 2. 分别将两种尺寸的data进行上传(upload)，获取对应的assetId
 3. 触发请求将旧的头像删除，并绑定新的头像，
 4. 更新conversation的属性，触发通知改变头像
 */
...
extension ConversationAvatarUpdateStatus {
    ...
    ///当前conversation处理结束，则从队列中移除，且如果队列中存在别的任务则开始进行
