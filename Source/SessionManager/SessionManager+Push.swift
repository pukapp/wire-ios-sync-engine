...
extension SessionManager: PKPushRegistryDelegate {
    ...
    public func pushRegistry(_ registry: PKPushRegistry, didReceiveIncomingPushWith payload: PKPushPayload, for type: PKPushType, completion: @escaping () -> Void) {
        ...
        print("pushRegistry----dictionaryPayload:\(payload.dictionaryPayload)")
        // 13.3以上只处理呼叫通话消息
        if #available(iOS 13.3, *) {
            // 呼叫消息
            if let _ = payload.dictionaryPayload.voipString(),
                let userId = payload.dictionaryPayload.accountId() {
                pushCallNotification(to: userId, payload: payload, completion: completion)
                
            } else { // 普通消息发到这里会崩溃
                Logging.push.safePublic("Received push payload none voipString or none accountId")
                completion()
            }
        } else { // 13.3以下按之前处理
            // 万人群消息推送
    ...
    private func pushCallNotification(to userUUID: UUID, payload: PKPushPayload, completion: @escaping () -> Void) {
        guard let voipString = payload.dictionaryPayload.voipString(),
              let voipModel = CallingVoipModel(json: JSON(parseJSON: voipString)) else {
            return completion()
        }
        ...
        callKitDelegate?.receiveVoipNotification(voipModel)
        completion()
    }
